...
test('new remix generation reserves a shimmering target frame before resolving', async ({ page }) => {
  ...
  await expect(page.locator('.variant-strip .variant-thumb').filter({ hasText: /Remix/ })).toBeVisible()
  ...
  await expect(page.getByLabel('Image generation prompt')).toContainText('fallbackPolicy')
  ...
  await expect(page.getByLabel('Generation observability stream')).toContainText('image_request')
  await expect(page.getByLabel('Generation observability stream')).toContainText('endpoint')
  await expect(page.getByLabel('Generation observability stream')).toContainText('/v1/images/edits')
  ...
  await expect(page.getByText('Raw SAM payload')).toHaveCount(0)
  await expect(page.getByText('Raw image payload')).toHaveCount(0)
  await expect(page.getByText('Raw composer request')).toHaveCount(0)
  ...
  await expect
    ...
    .toBeLessThan(4)
  await expect(page.getByLabel('Generation observability stream')).toContainText('Generated for')
  await expect(page.getByLabel('Generation observability stream')).toContainText('providerMode=mock')
  await expect(page.getByLabel('Generation observability stream')).toContainText('finalSegments')
  await expect(page.getByLabel('Generation observability stream')).toContainText('Emotional engagement')
  await expect(page.getByLabel('Generation observability stream')).not.toContainText(
